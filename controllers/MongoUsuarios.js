...
const { UsuarioMongo } = require("../models");
...
const usuariosGet = async (req = request, res = response) => {
    ...
    */
    ...
    const [total, usuarios] = await Promise.all([
        UsuarioMongo.countDocuments(query),
        UsuarioMongo.find(query)
        .populate("rol", "rol")
        .skip(Number(desde))
        .limit(Number(limite))
    ]);
...
const usuariosPost = async (req, res = response) => {
    ...
    const usuario = new UsuarioMongo(body);
...
const usuariosPut = async (req, res = response) => {
    ...
    try {
        ...
        */
        ...
        const usuario = await UsuarioMongo.findByIdAndUpdate(id, resto, { new: true });
...
const usuariosDelete = async (req, res = response) => {
    ...
    try {
        // Borrado Logico
        const usuario = await UsuarioMongo.findByIdAndUpdate(id, { estado: false, fecha_actualizacion: now() }, { new: true });
